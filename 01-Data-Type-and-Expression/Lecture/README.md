...
คำสั่ง `z = x + y` จะอ่านค่าที่ชื่อ `x` และ `y` ผูกอยู่มาบวกกัน แล้วผูกผลลัพธ์
กับชื่อ `z` บทนี้จะค่อย ๆ อธิบายความหมายของคำสั่งเหล่านี้

---

## 1.1. Variables

**Variables (ตัวแปร)** คือชื่อที่ใช้ผูก (bind) เข้ากับค่าในภาษา Python
การกำหนดค่าไม่ได้เป็นการเขียนสมการคณิตศาสตร์ แต่เป็นการบอกให้ชื่อทางซ้ายอ้างถึง
ค่าที่คำนวณได้ทางขวา ชื่อเดิมจึงสามารถผูกกับค่าใหม่ได้
...
- ใช้ตัวอักษร ตัวเลข หรือขีดล่าง (`_`) ได้ และห้ามขึ้นต้นด้วยตัวเลข
- ตัวอักษรพิมพ์ใหญ่และพิมพ์เล็กเป็นคนละชื่อกัน เช่น `score` กับ `Score`
- ห้ามใช้คำสงวน (keyword) ของภาษา Python เป็นชื่อตัวแปร

คำสงวนของ Python 3.11 มีดังนี้
...
สามารถตรวจรายการตามรุ่นของ Python ที่กำลังใช้ได้ด้วยโมดูล `keyword`
...
ใน Python 3.11 `keyword.kwlist` คือรายการคำสงวนในตาราง ส่วน
`keyword.softkwlist` ให้ `['_', 'case', 'match']` คำในรายการหลังเป็น
**soft keyword**: ใช้เป็นชื่อได้ในหลายบริบท แต่จะมีความหมายพิเศษในไวยากรณ์บางแบบ
เช่น `match`/`case`
...
**ข้อมูลประเภท `bool`** คือค่าความจริง มีสองค่า คือ `True` (จริง) และ
`False` (เท็จ)
...
**ข้อมูลประเภท `tuple`** คือรายการข้อมูลที่แก้ไขสมาชิกไม่ได้
...
**ข้อมูลประเภท `set`** คือรายการข้อมูลที่สมาชิกไม่ซ้ำกัน และไม่มีลำดับตายตัว
...
**Type Conversion (การแปลงประเภทข้อมูล)** คือการสร้างค่าใหม่ในประเภทที่ต้องการ
โดยใช้ฟังก์ชัน เช่น `int()` `float()` และ `str()`

**ฟังก์ชัน `int()`** แปลงค่าเป็นจำนวนเต็ม สำหรับ `float` จะตัดส่วนทศนิยมออก
โดยตัดเข้าหาศูนย์ (truncation toward zero) ไม่ใช่การปัดขึ้นหรือลงเสมอไป
...
ส่งค่าหลายค่าให้ `print()` ได้ โดยปกติ Python คั่นแต่ละค่าด้วยช่องว่างหนึ่งช่อง
...
> [!WARNING]
>
> `int(input())` จะเกิด `ValueError` ถ้าผู้ใช้กรอกข้อความที่ไม่ใช่จำนวนเต็ม เช่น
> `"ABCD"` หรือ `"10.5"` ส่วน `float(input())` รับ `"10.5"` ได้ แต่ยังเกิด
> `ValueError` กับข้อความอย่าง `"ABCD"`
...
**การหารปัดเศษ (Floor Division)** ใช้ `//` โดยผลลัพธ์คือค่าที่ปัดลงไปทาง
ลบอนันต์ (floor) จึงไม่ใช่เพียงการตัดทศนิยมออก
...
เมื่อมี `float` อย่าง `10.0 // 3` ผลลัพธ์เป็น `float` ด้วย ส่วน `-10 // 3` เป็น
`-4` เพราะ `-4` เป็นจำนวนเต็มที่ไม่มากกว่า `-10 / 3`
...
**Operations Precedence** คือลำดับการคำนวณของตัวดำเนินการ โดยสรุปได้ดังนี้

- วงเล็บ `()` ทำก่อน
- สำหรับฐานของเลขยกกำลัง `**` ทำก่อนเครื่องหมายบวกหรือลบแบบ unary
- เครื่องหมาย unary `+x` และ `-x` ทำก่อน `*`, `/`, `//`, `%`
- `*`, `/`, `//`, `%` มีความสำคัญเท่ากัน และทำจากซ้ายไปขวา
- `+`, `-` มีความสำคัญเท่ากัน และทำจากซ้ายไปขวา

การยกกำลังทำจากขวาไปซ้าย และมีลำดับที่ควรระวังกับเครื่องหมายลบ
...
ลำดับของ unary sign กับ `**` มีความไม่สมมาตร: `-2**2` หมายถึง `-(2**2)`
แต่ unary sign เขียนทางด้านเลขชี้กำลังได้ ดังนั้น `2**-2` หมายถึง `2**(-2)`
และมีค่า `0.25` หากต้องการยกกำลังของ `-2` ต้องเขียน `(-2)**2`

ตัวอย่างการคำนวณหลายตัวดำเนินการ
...
**Augmented Assignments** คือรูปย่อของการคำนวณแล้วกำหนดค่ากลับให้ตัวแปรเดิม
เช่น `a += 5` มีความหมายเดียวกับ `a = a + 5`
...
>
> **การทำซ้ำข้อความ:** `"ha" * 2.5` เกิด `TypeError` เพราะตัวคูณต้องเป็น
> `int` เช่น `"ha" * 2` การแปลง `2.5` เป็น `str` ไม่ช่วยให้ทำซ้ำข้อความได้

---

# Section 4: Math Module

Python มี **built-in functions** ที่เรียกใช้ได้ทันที และมีฟังก์ชันเพิ่มเติมใน
**module `math`** ซึ่งต้องนำเข้าก่อนใช้
...
**ฟังก์ชัน `max()`** และ **`min()`** หาค่ามากที่สุดและน้อยที่สุดจากตัวเลขที่
เปรียบเทียบกันได้
...
**ฟังก์ชัน `round(x, n)`** ปัด `x` โดย `n` เป็นอาร์กิวเมนต์ที่ไม่จำเป็น
หากไม่ระบุ `n` จะปัดเป็นจำนวนเต็มที่ใกล้ที่สุด
...
Python ปัดกรณีที่อยู่กึ่งกลางไปยังจำนวนคู่ที่ใกล้ที่สุด (เช่น `round(2.5)` เป็น
`2`) และค่าทศนิยมเป็นการประมาณในคอมพิวเตอร์ จึงไม่ควรคาดหวังการปัดแบบทศนิยม
ที่เหมือนการคำนวณด้วยมือทุกกรณี `round()` คืนค่าเป็นตัวเลข และไม่ได้บังคับให้
แสดงทศนิยมครบ `n` ตำแหน่ง เช่น `round(2, 3)` แสดงเป็น `2` ไม่ใช่ `2.000`
...
ฟังก์ชันต่อไปนี้อยู่ในโมดูล `math`
...
- `math.sqrt(x)` หารากที่สองของ `x`; สำหรับจำนวนจริง `x` ต้องไม่ติดลบ
- `math.ceil(x)` ปัดขึ้นไปยังจำนวนเต็มที่ไม่น้อยกว่า `x`
- `math.floor(x)` ปัดลงไปยังจำนวนเต็มที่ไม่มากกว่า `x`
- `math.factorial(n)` หา `n!`; `n` ต้องเป็นจำนวนเต็มที่ไม่ติดลบ
- `math.gcd(a, b)` หาห.ร.ม. และ `math.lcm(a, b)` หาค.ร.น. ของจำนวนเต็ม
...
- `math.pi` คือค่าประมาณของ π
- `math.e` คือค่าประมาณของ Euler's number
- `math.degrees(x)` แปลงมุม `x` จากเรเดียนเป็นองศา
- `math.radians(x)` แปลงมุม `x` จากองศาเป็นเรเดียน

ค่าที่แสดงเป็นทศนิยมเป็นค่าประมาณแบบ floating-point จึงอาจมีตัวเลขท้ายต่างจาก
ค่าทางคณิตศาสตร์ที่เขียนอย่างไม่สิ้นสุด
...
- `math.log(x, base)` คำนวณ log ฐาน `base`
- `math.log10(x)` และ `math.log2(x)` คำนวณ log ฐาน 10 และฐาน 2

> [!WARNING]
>
> อาร์กิวเมนต์ของฟังก์ชัน log ต้องมากกว่า 0 ดังนั้น `math.log(0)`,
> `math.log(-1)`, `math.log10(0)` และ `math.log2(-2)` เกิด `ValueError`
> สำหรับ `math.log(x, base)` นั้น `base` ต้องมากกว่า 0 และห้ามเท่ากับ 1

## 4.5. Trigonometry Functions

ฟังก์ชันตรีโกณมิติรับและคืนมุมเป็น **เรเดียน** ไม่ใช่องศา
...
> `math.sin(30)` โดยตรง ผลลัพธ์ของ `sin`, `cos` และ `tan` เป็นค่าประมาณแบบ
> floating-point จึงอาจมีทศนิยมคลาดเคลื่อนเล็กน้อย
